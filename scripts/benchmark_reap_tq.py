...
def main():
    ...
    if args.prune > 0:
        ...
        run_quality_check(model, tokenizer, "after REAP prune")
    else:
        mem_pruned = mem_bf16
    ...
    print(
        f"After TQ{args.bits}: {mem_compressed:.0f} MB ({mem_compressed / 1000:.1f} GB), "
    ...
    if args.prune > 0:
        print(f"After REAP:  {mem_pruned:.0f} MB ({mem_pruned / 1000:.1f} GB) [{args.prune * 100:.0f}% experts pruned]")
    print(
        f"After TQ{args.bits}:   {mem_compressed:.0f} MB ({mem_compressed / 1000:.1f} GB) "
        f"[{mem_bf16 / max(mem_compressed, 1):.1f}x total compression]"
    )
    print(f"Compress time: {compress_time:.1f}s" + (f" + {prune_time:.1f}s REAP" if args.prune > 0 else ""))
