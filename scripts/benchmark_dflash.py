...
def benchmark_autoregressive(model, tokenizer, prompts, max_new_tokens=256, label=""):
    ...
    logger.info("[%s] Total: %d tokens in %.1fs (%.1f tok/s), peak GPU: %.1f GB",
                label, total_tokens, total_time, avg_tok_s, peak_mem)
    return {
        "label": label,
...
def benchmark_dflash(draft_model, target, tokenizer, prompts,
                     max_new_tokens=256, temperature=0.0, label=""):
...
def main():
    ...
    if not args.skip_tq3_only:
        ...
        all_results["tq3"] = benchmark_autoregressive(
            model_tq3, tokenizer, prompts, args.max_new_tokens, label="TQ3"
        )
    ...
    if not args.skip_dflash_only:
        ...
        target_bf16 = AutoModelForCausalLM.from_pretrained(
            args.target, dtype=torch.bfloat16
        ).to(device).eval()
    ...
    for key, r in all_results.items():
        print(f"{r['label']:<20} {r['avg_tok_s']:>8.1f} {r['peak_gpu_gb']:>8.1f} GB "
