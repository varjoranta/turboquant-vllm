...
- **Pre-fused native MoE checkpoints** (v0.13.5): Qwen3.6-style native-packed checkpoints store the per-layer experts pre-fused on disk (`.experts.gate_up_proj.tq_packed`, `.experts.down_proj.tq_packed` — no per-expert index). The native MoE loader now detects this layout and yields directly to the placeholder param names, bypassing the per-expert regroup. Per-expert layout (Qwen3-30B-A3B) still works through the same code path. Adds mapper-aware name resolution for multimodal models (`hf_to_vllm_mapper` applied before lookup) so the regroup matches `language_model.model.…` post-mapper keys. Includes a defensive `Compressed3D.from_packed` meta guard and per-config scratch pool (no global state across multiple LLM instances). Validated on Qwen3.6-35B-A3B-TQ3-native on A100 80GB: **16.0 tok/s graphs-on, 9.81 tok/s eager** at bs=1 (vLLM 0.20.2). The CUDA graphs regression noted under v0.13.0 (eager-only on this stack) is resolved on vLLM 0.20+: graphs-on is now **1.6× faster** than eager rather than 3.3× slower. End-to-end pipeline (load, generate, eager + graphs-on, publish) verified across 4 independent runs; gsm8k-200 accuracy will be added as a follow-up — the eval workload exceeds the supervisor harness's polling window, not a quality concern.
...
- Targets T4/RTX 2080 (sm_75), A100 (sm_80), A10/A40/RTX 3090 (sm_86), L40S/RTX 4090 (sm_89), H100/H200 (sm_90), B200/GB200 (sm_100, CUDA ≥ 12.8), B300/GB300 (sm_103, CUDA ≥ 12.9), RTX 50xx (sm_120, CUDA ≥ 12.8), and GB10/DGX Spark (sm_121, CUDA ≥ 12.9). CUDA 13.x toolchains are supported (13.0 drops pre-Turing targets). When the local toolkit is too old for the GPU's native SASS (e.g. B200 under a CUDA 12.4 torch), the JIT build emits the newest supported PTX instead and the driver JIT-compiles it — builds without a visible GPU also embed forward-compatible PTX (`compute_90` on CUDA ≥ 11.8 toolchains, `compute_80` otherwise) so the same wheel/image runs on future architectures. Override targets with `TQ_CUDA_ARCH_LIST` (or `TORCH_CUDA_ARCH_LIST`). The PyTorch fallback runs on any CUDA GPU, CPU, or Apple MPS.
...
- **This plugin (`pip install turboquant-plus-vllm`)** — monkey-patches vLLM at import time via the `vllm.general_plugins` entry point. Validated on **vLLM 0.19–0.20**, and as of **v0.13.11** on **vLLM 0.25.x for dense models** (GPU-verified: Qwen3-8B `TQ_WEIGHT_BITS=3` starts, captures CUDA graphs, and decodes on an A100). Two vLLM-0.25 breaks were fixed getting there: engine init crashed on 0.25's non-class `FusedMoE` export (v0.13.10), and 0.25's default AOT compile (`VLLM_USE_AOT_COMPILE`) binds parameters by name and hit `KeyError: 'weight'` because the plugin replaces `.weight` with compressed buffers — v0.13.11 defaults AOT compile off (the regular piecewise-CUDA-graph path is unaffected). Online *MoE* weight compression is still skipped on 0.25.x (vLLM no longer exports `FusedMoE` as a plain class); native-packed MoE checkpoints are unaffected. Includes the CUDA bs=1 fused-dequant kernel; enabled through the `TQ_WEIGHT_BITS` env var; fully standalone.
...
The **3.07× live weight-memory compression** is real and matches the math. The **TQ3 decode throughput is ~7–17% of BF16** — a real performance gap that landed with the dynamo-fullgraph rewrite and is **not recoverable from host-side orchestration alone**. (Re-measured on **vLLM 0.25.0** with v0.13.11, A100 80GB, Qwen3-8B, bs=1 single-prompt decode: **6.98 vs 91.71 tok/s = 7.6% of BF16** — the gap persists on current vLLM, the bs=1 CUDA GEMV kernel notwithstanding.) The bottleneck is the Triton compressed-GEMM kernel itself: loading 3-bit codes, centroid lookup, rotation, FMA — all materially slower than cuBLAS BF16 on modern hardware. Recovery needs kernel-level work (better autotune configs, possibly a hybrid cuBLAS-GEMM + Triton-dequant path, or fusion into attention/MLP).
